...
export async function up(knex: Knex): Promise<any> {
    ...
    await knex.schema.createTable('products', (t) => {
        ...
        t.decimal('minimum_balance', 10, 2)
        t.boolean('has_overdraft')
        t.decimal('overdraft_limit', 10, 2)
        t.decimal('overdraft_interest_rate', 10)
        t.integer('overdraft_tenor')
        t.decimal('deposit_interest_rate', 10)
        t.integer('deposit_tenor')
        t.decimal('institution_percentage', 10)
        t.decimal('customer_percentage', 10)
